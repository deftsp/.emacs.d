...
;; info files.
;;
;; The documentation is expected to be HTML files with devhelp indexes, like
;; the Debian packages libgtk2.0-doc etc.  See the `gtk-lookup-symbol'
...
Usually these files are under /usr/share/gtk-doc/html, and possibly
  ...
  (when (eq gtk-lookup-cache 'uninitialized)
    ...
    (let ((found nil))
      (gtk-lookup-with-auto-compression
       (with-temp-buffer
         (let ((filelist
                ;; file-truename here and delete below eliminate any
                ;; duplicate filenames arising from symlinks or repeat
...
\"parent\" doesn't match \"gtk_widget_unparent\"."
  ...
  (setq method (concat "_" method)) ;; at _ boundary, and proper suffix
  (let (ret)
    ...
    (let ((case-fold-search nil))
      ...
      (if (string-match "[_-]" str)
          (progn
            ...
            ;; and "UIManager" -> "UI_Manager"
            (while (string-match "\\([A-Za-z]\\)\\([A-Z][a-z]\\)" str)
...
applied (to turn it into a C name) designed to support
...
lookup from the browser for symbols without HTML links (eg. cross
references from Gtk to Pango).
    ...
    (let ((other-window (get-buffer-window dummy-buf)))
      ...
      (cond ((and (eq dummy-buf (window-buffer other-window))
            ...
            ((eq orig-buffer (window-buffer other-window))
             ...
             ;; window configs back how they were, but keep point in the
             ;; browser buffer as it's at the new document position
