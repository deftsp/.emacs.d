...
(defface aw-mode-line-face '()
  "Face used for acw window key in the mode-line."
  :group 'ace-window)

(defvar aw-table nil "window and key hash map")

(defvar ace-window-mode-line-format "#%c" "ace window mode-line format.")

(defvar ace-window-mode-line-position 1
...
(defun aw-update ()
  (let* ((key-list aw-keys)
         ...
         (win-list
          (mapcar (lambda (va) (aj-visual-area-window va))
                  visual-area-list)))
    (clrhash aw-table)
    (dolist (win win-list)
...
(defun aw-install-mode-line (&optional position)
  "Install the window number from `ace-window-mode' to the mode-line."
