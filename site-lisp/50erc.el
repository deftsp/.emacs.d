...
;;; modules
(setf erc-modules '(autoaway autojoin button completion fill irccontrols
                    log match menu netsplit noncommands
                    ;; services ; Identify to Nickserv (IRC Services) automatically
                    ;; bbdb smiley page
                    notify readonly ring scrolltobottom stamp track))
          ...
          '(lambda (SERVER NICK)
            (cond
              ((string-match "freenode\\.net" SERVER)
               (erc-message "PRIVMSG" "NickServ identify 0o0oAa"))
              ((string-match "oftc\\.net" SERVER)
               (erc-message "PRIVMSG" "NickServ identify 0o0oAa")))))

(setq erc-autojoin-channels-alist '(("freenode.net" "#emacs" "#stumpwm" "#gentoo-cn")
                                    ;; ("oftc.net" "#debian" "#debian-zh")
                                    ))
...
(defvar bitlbee-password "0o0oAa")
...
(setq erc-nick "deftsp"
      erc-nick-uniquifier "0"
      erc-email-userid "deftsp@gmail.com>"
...
(setq  erc-server-coding-system '(utf-8 . utf-8)
       erc-encoding-coding-alist '(("#linuxfire" . chinese-iso-8bit) ; this is an alist and you can append cons cell for multi-channel
...
(define-key erc-mode-map (kbd "M-q") 'tsp-unfill-line)
...
(define-key erc-mode-map [(meta ?g)] 'tsp-switch-to-buffer)
...
(global-set-key (kbd "C-c n e") 'tsp-toggle-erc-busy)
