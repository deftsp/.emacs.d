;;; core-jump.el --- Paloryemacs Core File
;;
...
(defvar paloryemacs-default-jump-handlers '()
  "List of jump handlers available in every mode.")

(defvar-local paloryemacs-jump-handlers '()
  "List of jump handlers local to this buffer.")

(defmacro paloryemacs|define-jump-handlers (mode &rest handlers)
  "Defines jump handlers for the given MODE.
This defines a variable `paloryemacs-jump-handlers-MODE' to which
handlers can be added, and a function added to MODE-hook which
sets `paloryemacs-jump-handlers' in buffers of that mode."
  (let ((mode-hook (intern (format "%S-hook" mode)))
        (func (intern (format "paloryemacs//init-jump-handlers-%S" mode)))
        (handlers-list (intern (format "paloryemacs-jump-handlers-%S" mode))))
    `(progn
       (defvar ,handlers-list ',handlers
         ,(format (concat "List of mode-specific jump handlers for %S. "
                          "These take priority over those in "
                          "`paloryemacs-default-jump-handlers'.")
                  mode))
       (defun ,func ()
         (setq paloryemacs-jump-handlers
               (append ,handlers-list
                       paloryemacs-default-jump-handlers)))
       (add-hook ',mode-hook ',func)
       (with-eval-after-load 'bind-map
         (paloryemacs/set-leader-keys-for-major-mode ',mode
           "gg" 'paloryemacs/jump-to-definition
           "gG" 'paloryemacs/jump-to-definition-other-window)))))

(defun paloryemacs/jump-to-definition ()
  "Jump to definition around point using the best tool for this action."
  (interactive)
  (catch 'done
    (let ((old-buffer (current-buffer))
          (old-point (point)))
      (dolist (-handler paloryemacs-jump-handlers)
...
(defun paloryemacs/jump-to-definition-other-window ()
  "Jump to definition around point in other window."
  (interactive)
  (let ((pos (point)))
    ;; since `paloryemacs/jump-to-definition' can be asynchronous we cannot use
    ;; `save-excursion' here, so we have to bear with the jumpy behavior.
    (switch-to-buffer-other-window (current-buffer))
    (goto-char pos)
    (paloryemacs/jump-to-definition)))
...
(with-eval-after-load 'evil
  (evil-set-command-property 'paloryemacs/jump-to-definition :jump t))
