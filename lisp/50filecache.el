...
(defvar tl/file-cache-dirs '("~/" "/etc/" "~/.emacs.d/lisp/"))
(defvar tl/file-cache-recursive-dirs '("~/org"))

(defun tl/filecache-add-files ()
  (ignore-errors (file-cache-add-directory-list tl/file-cache-dirs)
   ;; (file-cache-add-directory-list load-path)
   (let ((cache-dirs tl/file-cache-recursive-dirs))
...
(eval-after-load "filecache"
  '(progn
     ...
     (run-at-time (tl/future-time-string 60) (* 12 60 60) 'tl/filecache-add-files)
...
(add-hook 'kill-buffer-hook 'tl/file-cache-add-this-file)
