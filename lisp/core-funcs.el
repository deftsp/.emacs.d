...
(defvar dotpaloryemacs-filepath)
(defvar paloryemacs-repl-list '()
  "List of all registered REPLs.")

(defun paloryemacs/system-is-mac ()
  (eq system-type 'darwin))
(defun paloryemacs/system-is-linux ()
  (eq system-type 'gnu/linux))
(defun paloryemacs/system-is-mswindows ()
  (eq system-type 'windows-nt))

(defun paloryemacs/window-system-is-mac ()
  ;; ns is returned instead of mac on Emacs 25+
  (memq (window-system) '(mac ns)))

(defun paloryemacs/run-prog-mode-hooks ()
  "Runs `prog-mode-hook'. Useful for modes that don't derive from
`prog-mode' but should."
  (run-hooks 'prog-mode-hook))

(defun paloryemacs/run-text-mode-hooks ()
  "Runs `text-mode-hook'. Useful for modes that don't derive from
`text-mode' but should."
  (run-hooks 'text-mode-hook))

(defun paloryemacs/mplist-get (plist prop)
...
(defun paloryemacs/mplist-remove (plist prop)
...
(defun paloryemacs/dump-vars-to-file (varlist filename)
  "simplistic dumping of variables in VARLIST to a file FILENAME"
  (with-temp-file filename
    (paloryemacs/dump varlist (current-buffer))
    (make-directory (file-name-directory filename) t)))

;; From http://stackoverflow.com/questions/2321904/elisp-how-to-save-data-in-a-file
(defun paloryemacs/dump (varlist buffer)
  "insert into buffer the setq statement to recreate the variables in VARLIST"
  (cl-loop for var in varlist do
           (print (list 'setq var (list 'quote (symbol-value var)))
                  buffer)))

(defvar paloryemacscemacs--init-redisplay-count 0
  "The paloryemacs calls to `redisplay'")
(defun paloryemacs//redisplay ()
  "`redisplay' wrapper."
  (setq paloryemacs--init-redisplay-count (1+ paloryemacs--init-redisplay-count))
  (redisplay))

(defun paloryemacs//create-key-binding-form (props func)
  "Helper which returns a from to bind FUNC to a key according to PROPS.

Supported properties:

`:evil-leader STRING'
    One or several key sequence strings to be set with `paloryemacs/set-leader-keys .
...
`:define-key CONS CELL'
  ...
  (let ((evil-leader (paloryemacs/mplist-get props :evil-leader))
        (evil-leader-for-mode (paloryemacs/mplist-get props :evil-leader-for-mode))
        (global-key (paloryemacs/mplist-get props :global-key))
        (def-key (paloryemacs/mplist-get props :define-key)))
    (append
     (when evil-leader
       `((dolist (key ',evil-leader)
           (paloryemacs/set-leader-keys key ',func))))
...
(defun paloryemacss/prettify-org-buffer ()
...
(defun paloryemacs/view-org-file (file &optional anchor-text expand-scope)
...
If EXPAND-SCOPE is `all' then run `outline-show-all' at the matched line."
  (interactive)
  (find-file file)
  (paloryemacs/prettify-org-buffer)
  (goto-char (point-min))
  (when anchor-text
...
(defun paloryemacs//test-var (pred var test-desc)
...
passed-tests and total-tests."
  (let ((var-name (symbol-name var))
    ...
    (insert (format "** TEST: [[file:%s::%s][%s]] %s\n"
                    dotpaloryemacs-filepath var-name var-name test-desc))
...
(defun paloryemacs//test-list (pred varlist test-desc &optional element-desc)
  ...
  (let ((varlist-name (symbol-name varlist))
    ...
    (if element-desc
        (insert (format "** TEST: Each %s in [[file:%s::%s][%s]] %s\n"
                        element-desc dotpaloryemacs-filepath varlist-name
                        varlist-name test-desc))
      (insert (format "** TEST: Each element of [[file:%s::%s][%s]] %s\n"
                      dotpaloryemacs-filepath varlist-name varlist-name
                      test-desc)))
...
(defun paloryemacs/recompile-elpa ()
  "Recompile packages in elpa directory. Useful if you switch
Emacs versions."
  (interactive)
  (byte-recompile-directory package-user-dir nil t))

(defun paloryemacs/register-repl (feature repl-func &optional tag)
  "Register REPL-FUNC to the global list of REPLs PALORYEMACS-REPL-LIST.
...
(defun paloryemacs/add-to-hooks (fun hooks)
  "Add function to hooks"
  (dolist (hook hooks)
    (add-hook hook fun)))

(defun paloryemacs/add-all-to-hook (hook &rest funs)
  "Add functions to hook."
  (paloryemacs/add-to-hook hook funs))

(defun paloryemacs/add-to-hook (hook funs)
  "Add list of functions to hook."
  (dolist (fun funs)
    (add-hook hook fun)))

(defun paloryemacs/echo (msg &rest args)
  "Display MSG in echo-area without logging it in *Messages* buffer."
  (interactive)
  (let ((message-log-max nil))
    (apply 'message msg args)))

(defun paloryemacs/derived-mode-p (mode &rest modes)
...
(defun paloryemacs/alternate-buffer (&optional window)
...
(defun paloryemacs/alternate-window ()
...
(defvar-local paloryemacs--gne-min-line nil
  "The first line in the buffer that is a valid result.")
(defvar-local paloryemacs--gne-max-line nil
  "The last line in the buffer that is a valid result.")
(defvar-local paloryemacs--gne-cur-line 0
  "The current line in the buffer. (It is problematic to use
point for this.)")
(defvar-local paloryemacs--gne-line-func nil
  "The function to call to visit the result on a line.")

(defun paloryemacs/gne-next (num reset)
  "A generalized next-error function. This function can be used
as `next-error-function' in any buffer that conforms to the
Paloryemacs generalized next-error API.

The variables `paloryemacs--gne-min-line',
`paloryemacs--gne-max-line', and `paloryemacs--line-func' must be
set."
  (when reset (setq paloryemacs--gne-cur-line
                    paloryemacs--gne-min-line))
  (setq paloryemacs--gne-cur-line
        (min paloryemacs--gne-max-line
             (max paloryemacs--gne-min-line
                  (+ num paloryemacs--gne-cur-line))))
  (goto-line paloryemacs--gne-cur-line)
  (funcall paloryemacs--gne-line-func
