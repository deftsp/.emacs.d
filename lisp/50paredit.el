...
(eval-after-load 'paredit
  '(progn
     ...
     (define-key paredit-mode-map (kbd "<delete>") 'tl/paredit-forward-maybe-delete-region)
     (define-key paredit-mode-map (kbd "DEL") 'tl/paredit-backward-maybe-delete-region)))
...
(defun tl/paredit-backward-maybe-delete-region ()
  (interactive)
  (if mark-active
      (progn
        (check-region-parens)
...
(defun tl/paredit-forward-maybe-delete-region ()
  (interactive)
  (if mark-active
      (progn
        (check-region-parens)
...
(defun tl/paredit-wrap-round-from-behind ()
  (interactive)
  (forward-sexp -1)
  (paredit-wrap-round)
  (insert " ")
